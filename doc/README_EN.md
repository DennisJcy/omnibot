<p align="center">
  <img src="../images/logo.png" alt="omnibot Icon" width="120" height="120" />
</p>
...
<p align="center">
omnibot is a bridge service that connects MCP clients to real browser sessions. Through its companion Chromium extension, any LLM client can seamlessly list tabs, scan pages, execute automation, capture screenshots, and navigate pages.
</p>

# Introduction

omnibot is designed for scenarios where large language models need to operate a real browser. **Unlike stateless HTTP fetching, it connects to browser pages that are already open and already logged in, so it can reuse the browser's login state, cookies, page state, and rendered frontend result.**

omnibot also supports multi-profile operations on a single machine and multi-user operations.

> This project is written and distributed in Python. MCP supports two transport modes: `stdio` and `streamable-http`.

# Project Advantages

**Why use omnibot instead of Playwright MCP, Kimi Bridge, or Chrome DevTools MCP?**
...
omnibot has a different goal: it focuses on letting LLMs or Agent products take over the real browser session the user is already using.

- **Reuse real login state**: omnibot connects to browser tabs that are already open and logged in. For many account-based websites, there is no need to log in again or manually transfer cookies.
- **Better for everyday browser collaboration**: Playwright is a strong fit for repeatable, scriptable automation workflows, while omnibot is better suited for interactive tasks where an LLM reads, analyzes, checks before clicking, executes scripts, and takes screenshots on the user's current page.
...
So if your goal is to let a model control a dedicated automation browser, Playwright MCP is a good fit. If your goal is to debug Chrome or work closely with the DevTools protocol, Chrome DevTools MCP is a good fit. If your goal is to let a model or Agent product read and operate on the real browser page the user is currently using, omnibot is closer to that scenario.

## System Architecture

<p align="center">
  <img src="./architecture.png" alt="omnibot system architecture" />
</p>
...
graph TB
    ...
    subgraph Server["⚙️ omnibot MCP Server (Python)"]
...
1. The MCP client connects to the omnibot service via **stdio** (subprocess) or **streamable-http** (HTTP endpoint). In `streamable-http` mode, the client specifies its user context with the `Authorization: Bearer <token>` header.
...
If the MCP client server key is `omni`, tool calls are typically displayed as `omnibot_navigate`, `omnibot_scan`, and so on.
...
Download the executable for your platform from [GitHub Releases](https://github.com/DennisJcy/omnibot/releases).

### MCP Configuration

**After npm global install (recommended):**
...
**After manually downloading the executable:**

1. **Download**: Go to the [Releases](https://github.com/DennisJcy/omnibot/releases) page, download the archive for your OS and extract it to a known path.
2. Open `chrome://extensions/` in Chrome or another Chromium browser and enable "Developer mode".
3. Click "Load unpacked" and select the `browser-extension` folder (obtain it from the source repo).
4. Add `omni` to your MCP client and configure the executable path.
...
```json
{
  "mcpServers": {
    "omni": {
      "command": "C:/tools/omnibot/omnibot.exe"
    }
  }
}
```
...
1. Load the browser extension folder `browser-extension` into Chrome or another Chromium-based browser.
2. Configure omnibot in your MCP client.
...
<p align="center">
  <img src="./plugin.png" alt="omnibot browser extension popup" width="360" />
</p>
...
omnibot can be started via npm (no Python required).

### Two Transport Modes

omnibot supports two MCP transport modes. Choose based on your use case:
...
omnibot --ws-port 18767

# streamable-http mode with a custom MCP HTTP port
omnibot --transport streamable-http --port 8000

# streamable-http mode with both MCP HTTP port and browser-extension WebSocket port
omnibot --transport streamable-http --port 8000 --ws-port 18767

# streamable-http mode, only allow specific tokens
omnibot --transport streamable-http --port 8000 --tokens "team_alice,team_bob"

# You can also pass the token whitelist via environment variable
OMNIBOT_TOKENS="team_alice,team_bob" omnibot --transport streamable-http --port 8000
```
...
**stdio mode (npm global install, recommended):**
...
claude mcp add omni -- omnibot

# streamable-http mode (start the service first, then register it)
claude mcp add omni --transport streamable-http http://127.0.0.1:8000/mcp
```
...
```bash
# npm global install (recommended)
codex mcp add omni -- omnibot

# streamable-http mode
codex mcp add omni --transport streamable-http --url http://127.0.0.1:8000/mcp
```
