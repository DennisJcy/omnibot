...
function createEnhancedDOMCopy() {  
  ...
  function cloneNode(sourceNode, keep=false) {  
    ...
    const isDropdown = sourceNode.classList?.contains('dropdown-menu') ||   
              /dropdown|menu/i.test(String(sourceNode.className || sourceNode.getAttribute('class') || '')) || sourceNode.getAttribute('role') === 'menu'; 
    ...
    const shadowRoot = sourceNode.shadowRoot || sourceNode.__omnibotClosedShadowRoot;
    if (shadowRoot) {
...
js_findMainList = r'''function findMainList(startElement = null) {
    ...
    function findTopGroups(container, limit) {
        ...
        children.forEach(child => {
            ...
            // 统计类名
            if (child.className) {
                const clsStr = String(typeof child.className === 'string' ? child.className : (child.className.baseVal || child.className.animVal || ''));
                clsStr.trim().split(/\s+/).forEach(cls => {
                    if (cls) {
        ...
        topTags.forEach(tag => {
            topClasses.forEach(cls => {
                const elements = children.filter(el =>
                                                 ...
                                                 el.classList && el.classList.contains(cls)
                                                );
        ...
        for (let i = 0; i < topClasses.length; i++) {
            for (let j = i + 1; j < topClasses.length; j++) {
                const elements = children.filter(el =>
                                                 el.classList && el.classList.contains(topClasses[i]) && el.classList.contains(topClasses[j]));
...
import time, re, os
def get_main_block(driver, extra_js="", text_only=False, token=None, group_status=None, session_id=None, status_tab_id=None):
    page = driver.execute_js(f"{extra_js}\n{js_optHTML}\nreturn optHTML({str(text_only).lower()});", token=token, group_status=group_status, session_id=session_id, status_tab_id=status_tab_id).get('data', '')
    if text_only:
...
def _control_label(control):
    if control.get("aria-label"):
        return _clean_text(control.get("aria-label"))
    if control.get("id"):
        label = control.find_parent().find("label", attrs={"for": control.get("id")}) if control.find_parent() else None
        if label:
            return _clean_text(label.get_text(" ", strip=True))
    if control.name == "button":
        return _clean_text(control.get_text(" ", strip=True))
    return ""


def _extract_repeated_items(soup, max_groups=5, max_items=6):
    groups = []
    selectors = ["article", "li", "tr"]
    for selector in selectors:
        items = soup.select(selector)
        meaningful = []
        for item in items:
            text = _clean_text(item.get_text(" ", strip=True), 300)
            if len(text) < 20:
                continue
            title_el = item.find(["h1", "h2", "h3", "h4", "a"])
            link_el = item.find("a", href=True)
            meaningful.append({
                "title": _clean_text(title_el.get_text(" ", strip=True) if title_el else text, 120),
        ...
        if len(meaningful) >= 2:
            groups.append({
                "selector": selector,
                "item_count": len(meaningful),
                "items": meaningful[:max_items],
            })
...
def extract_page_structure(html: str, text_only: bool = False) -> dict:
    """Extract task-oriented page metadata from simplified page HTML.

    This is intentionally conservative: it gives agents a structured first look
    at the page so they do not need JavaScript for common reading tasks.
    """
    soup = BeautifulSoup(html or "", "html.parser")
    headings = []
    for heading in soup.find_all(["h1", "h2", "h3", "h4", "h5", "h6"], limit=40):
        text = _clean_text(heading.get_text(" ", strip=True), 160)
        if text:
    ...
    controls = []
    for control in soup.select("input:not([type=hidden]), textarea, select, button")[:40]:
        controls.append({
            "tag": control.name,
            ...
            "value": control.get("value", "") or control.get("data-selected", ""),
        })

    forms = []
    for form in soup.find_all("form", limit=10):
        names = []
        for control in form.select("input:not([type=hidden]), textarea, select, button"):
            ident = control.get("name") or control.get("id") or _control_label(control)
            if ident:
                names.append(_clean_text(ident, 80))
        forms.append({
    ...
    return {
        ...
        "controls": controls,
        "forms": forms,
        "list_candidates": [] if text_only else _extract_repeated_items(soup),
    }


def get_html(driver, cutlist=False, maxchars=35000, instruction="", extra_js="", text_only=False, token=None, group_status=None, session_id=None, status_tab_id=None):
    ...
    page = get_main_block(driver, extra_js=extra_js, text_only=text_only, token=token, group_status=group_status, session_id=session_id, status_tab_id=status_tab_id)
...
def execute_js_rich(script, driver, no_monitor=False, token=None, group_status=None, session_id=None, status_tab_id=None):
    last_html = None
    if not no_monitor:
        try: last_html = get_html(driver, cutlist=False, extra_js=temp_monitor_js, maxchars=9999999, token=token, session_id=session_id, status_tab_id=status_tab_id)
        except: pass
    ...
    try:
        ...
        response = driver.execute_js(script, token=token, group_status=group_status, session_id=session_id, status_tab_id=status_tab_id)
    ...
    if response.get('newTabs'): rr['newTabs'] = response['newTabs']
        ...
        if new_sids:
            # Session-set differences have no causal ownership information.
            # A user can open an unrelated tab while this command is running,
            # so expose these as observations only; never feed them into the
            # tool-created-tab auto-close lifecycle.
            rr['observedTabs'] = [{'id': k, 'url': v} for k, v in new_sids.items()]
            rr['suggestion'] = "执行期间观察到新标签页；来源未经确认，不会自动关闭。"
    ...
    if not reloaded and len(newTabs) == 0:
        try:
            current_html = get_html(driver, cutlist=False, maxchars=9999999, token=token, session_id=session_id, status_tab_id=status_tab_id)
